...
def apply_instruction(state, instruction):
    ...
    state = operate_crane(state, initial_stack, end_stack, num_crates)
    
    return state

def operate_crane(state, initial, destination, num_crates):
    initial -= 1
    destination -= 1

    payload = state[initial][-num_crates:]
    state[initial] = state[initial][:-num_crates]

    print(f"Moving {num_crates} crate(s) [{payload}] from stack {initial+1} to {destination+1}")
